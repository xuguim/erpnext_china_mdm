...
def has_query_permission(user):
	if frappe.db.get_value('Has Role',{'parent':user,'role':['in',['System Manager','财务管理']]}):
		# 如果角色包含管理员，则看到全量
		conditions = ''
	elif frappe.db.get_value('Has Role',{'parent':user,'role':['in',['销售会计','销售支持']]}):
		# 如果角色包含管理员，则看到全量
		conditions = f"`custom_original_sales_order` is null"
	else:
